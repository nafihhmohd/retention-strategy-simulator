...
if page == "🏠  Project Overview":
    ...
    nav_pages = [
        ...
        ("🎯", "Strategy Simulator", "Discount vs Loyalty vs Engagement · ROI"),
        ("🔍", "Customer Lookup", "Live churn prediction · CLV · Recommendation"),
    ]
    nav_cols = st.columns(4)
    for col, (icon, title, desc) in zip(nav_cols, nav_pages):
        with col:
    ...
    if "nav_page" in st.session_state:
        mapping = {
            ...
            "CLV & Revenue Risk": "💰  CLV & Revenue Risk",
            "Strategy Simulator": "🎯  Strategy Simulator",
            "Customer Lookup":    "🔍  Customer Lookup",
        }
        target = mapping.get(st.session_state.pop("nav_page"), None)
        if target:
            st.info(f"👈 Select **{target}** from the sidebar to navigate there.")
