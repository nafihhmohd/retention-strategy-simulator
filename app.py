...
def compute_strategy_summary(df):
    # Step 2.1: Your exact strategy definitions
...
if page == "🏠  Project Overview":
    ...
    # Clickable nav buttons — directly update session_state and rerun
    nav_pages = [
        ("📉", "📉  Churn Analysis",      "Logistic Regression · Risk tiers"),
        ("💰", "💰  CLV & Revenue Risk",  "BG/NBD · Gamma-Gamma · Top 20"),
        ("🎯", "🎯  Strategy Simulator",  "Discount vs Loyalty vs Engagement"),
        ("🔍", "🔍  Customer Lookup",     "Live churn prediction · CLV"),
    ]
    nav_cols = st.columns(4)
    for col, (icon, full_page, desc) in zip(nav_cols, nav_pages):
        with col:
            if st.button(f"{icon}  {full_page.split('  ')[1]}", key=f"nav_{full_page}", use_container_width=True):
...
# ══════════════════════════════════════════════════════════════
    ...
    # Strategy definitions from your notebook
    base_strategies = {
